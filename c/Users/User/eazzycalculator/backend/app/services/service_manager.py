    ...
    def get_service(cls, service_name: str, service_class: Type, *args, **kwargs) -> Any:
        ...
        with cls._lock:
            ...
            if service_name in cls._instances:
                print(f"✅ Service '{service_name}' déjà existant - réutilisation")
                return cls._instances[service_name]
            
            # Créer nouvelle instance
            try:
                print(f"🔄 Création nouveau service '{service_name}'")
                instance = service_class(*args, **kwargs)
                cls._instances[service_name] = instance
                print(f"✅ Service '{service_name}' créé avec succès")
                return instance
                
            except Exception as e:
                print(f"❌ Erreur création service '{service_name}': {e}")
                raise
